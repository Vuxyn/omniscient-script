...
class Image_Ops:
    ...
    @staticmethod
    def diagonal_split(image: ArrayLike) -> tuple[ArrayLike, ArrayLike]:
        """Splits an image into lower-diagonal (bottom) and upper-diagonal (top) parts."""
        image = _validate_image(image)
        xp_mod = _xp(image)
        h, w = image.shape[:2]
        
        # Create coordinates
        y, x = xp_mod.ogrid[:h, :w]
        mask_bottom = y >= x
        
        if image.ndim == 3:
            mask_bottom = xp_mod.repeat(mask_bottom[..., None], image.shape[2], axis=2)
            
        bottom = xp_mod.where(mask_bottom, image, 0)
        top = xp_mod.where(~mask_bottom, image, 0)
        return bottom, top

    @staticmethod
    def diagonal_combine(bottom: ArrayLike, top: ArrayLike) -> ArrayLike:
        """Combines two images along the diagonal. bottom used for i > j, top for i <= j."""
        bottom = _validate_image(bottom)
        top = _validate_image(top)
        xp_mod = _xp(bottom)
        h, w = bottom.shape[:2]
        ...
        if bottom.ndim == 3:
            mask_bottom = xp_mod.repeat(mask_bottom[..., None], bottom.shape[2], axis=2)
            
        return xp_mod.where(mask_bottom, bottom, top)

    @staticmethod
    def normalize_min_max(image: ArrayLike) -> ArrayLike:
        """Linear min-max normalization to [0, 255]."""
        image = _validate_image(image)
        xp_mod = _xp(image)
        img_f = image.astype(xp_mod.float32)
        mi, ma = img_f.min(), img_f.max()
        if xp_mod.isclose(mi, ma):
            return xp_mod.zeros_like(image)
        norm = (img_f - mi) / (ma - mi)
        return (norm * 255).astype(image.dtype)
...
class Specialization:
    ...
    @staticmethod
    def selective_match(image: ArrayLike, reference: ArrayLike, mask: ArrayLike) -> ArrayLike:
        """Applies histogram matching only to the regions where mask is non-zero (white)."""
        image = to_cpu(_validate_image(image))
        reference = to_cpu(_validate_image(reference))
        mask = to_cpu(_validate_image(mask))
        
        matched = Specialization.match(image, reference)
        
        # Normalize mask to boolean
        mask_bool = mask > 127 if mask.max() > 1 else mask > 0.5
        
        if image.ndim == 3 and mask_bool.ndim == 2:
            mask_bool = np.repeat(mask_bool[..., None], image.shape[2], axis=2)
            
        return np.where(mask_bool, matched, image)
